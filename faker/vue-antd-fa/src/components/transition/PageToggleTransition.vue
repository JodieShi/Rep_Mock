...
export default {
  ...
  props: {
    ...
    direction: {
      type: String,
      validator(value) {
        return ['x', 'y', 'left', 'right', 'up', 'down', 'downLeft', 'upRihgt', 'downRight', 'upLeft',
          'downBig', 'upBig', 'topRight', 'bottomLeft', 'topLeft', 'bottomRight', 'default'].indexOf(value) > -1
      }
    },
    reverse: {
      type: Boolean,
  ...
  methods: {
    activeClass(isLeave) {
      let animate = animates.find(item =>item.name == this.animate)
      ...
      return animate.name + t + direction  // animate.css中的动画效果
    },
